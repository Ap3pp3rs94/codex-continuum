...
Describe "Codex Continuum package" {
    ...
    It "keeps the confirmed submit fallback in the watcher" {
        ...
        foreach ($pattern in @(
            ...
            "TargetResumeGraceSeconds",
            "TargetResumePollSeconds",
            "SuspendGapSeconds",
            "PostResumeSettleSeconds",
            "Get-StatusSnapshotHash",
            "Sync-LiveSessionHandle",
            "Resolve-ResumedLiveSessionHandle",
            "Normalize-LiveWindowTitleForResume",
            ...
            "codex_live_continue.resume_settle_complete",
    ...
    It "reports target-resume pause receipts" {
        $statusScript = Join-Path $repoRoot "scripts\get-continuum-status.ps1"
        $receipt = Join-Path ([System.IO.Path]::GetTempPath()) "codex-continuum-target-resume-test-$PID.jsonl"
        $now = [DateTimeOffset]::Now
        @(
            ([ordered]@{
                ts = $now.AddSeconds(-20).ToString("o")
                event = "codex_live_continue.attached"
                target_process_id = 0
                session_id = "target-resume-test-session"
                handle = "0x1"
                title = "Francis"
                title_working_pattern = "(^|:\s*)[\u280b]\s+"
            } | ConvertTo-Json -Compress),
            ([ordered]@{
                ts = $now.AddSeconds(-10).ToString("o")
                event = "codex_live_continue.target_missing_paused"
                handle = "0x1"
                title = "Francis"
                resume_title_key = "Francis"
                target_process_id = 1234
                target_resume_grace_s = 0
                target_resume_grace_unlimited = $true
                target_resume_poll_s = 10
                candidate_count = 0
        ...
        try {
            $json = & $statusScript -ReceiptPath $receipt -Json
            $summary = ($json | Out-String) | ConvertFrom-Json
            if ($summary.TargetResumePause.Active -ne $true) {
                throw "Status did not report active target-resume pause."
            }

            if ($summary.TargetResumePause.GraceUnlimited -ne $true) {
                throw "Status did not report unlimited target-resume grace."
            }

            if ($summary.TargetResumePause.Reason -ne "no_resume_candidate") {
                throw "Status did not report target-resume pause reason."
            }
        }
        finally {
            Remove-Item -LiteralPath $receipt -Force -ErrorAction SilentlyContinue
    ...
    It "clears target-resume pause state after target resumes" {
        $statusScript = Join-Path $repoRoot "scripts\get-continuum-status.ps1"
        $receipt = Join-Path ([System.IO.Path]::GetTempPath()) "codex-continuum-target-resumed-test-$PID.jsonl"
        $now = [DateTimeOffset]::Now
        @(
            ([ordered]@{
                ts = $now.AddSeconds(-30).ToString("o")
                event = "codex_live_continue.attached"
                target_process_id = 0
                session_id = "target-resume-test-session"
                handle = "0x1"
            ...
            ([ordered]@{
                ts = $now.AddSeconds(-10).ToString("o")
                event = "codex_live_continue.target_resumed"
                handle = "0x2"
                old_handle = "0x1"
                title = "Francis"
                resume_title_key = "Francis"
        ...
        try {
            $json = & $statusScript -ReceiptPath $receipt -Json
            $summary = ($json | Out-String) | ConvertFrom-Json
            if ($summary.TargetResumePause.Active -ne $false) {
                throw "Status kept target-resume pause active after target resumed."
            }
        }
        finally {
            Remove-Item -LiteralPath $receipt -Force -ErrorAction SilentlyContinue
    ...
    It "reports suspend-gap settle receipts" {
        $statusScript = Join-Path $repoRoot "scripts\get-continuum-status.ps1"
        $receipt = Join-Path ([System.IO.Path]::GetTempPath()) "codex-continuum-suspend-gap-test-$PID.jsonl"
        $now = [DateTimeOffset]::Now
        $settleUntil = $now.AddMinutes(5)
        @(
            ([ordered]@{
                ts = $now.AddSeconds(-20).ToString("o")
                event = "codex_live_continue.attached"
                target_process_id = 0
                session_id = "suspend-gap-test-session"
                handle = "0x1"
                title_working_pattern = "(^|:\s*)[\u280b]\s+"
            } | ConvertTo-Json -Compress),
            ([ordered]@{
                ts = $now.AddSeconds(-10).ToString("o")
                event = "codex_live_continue.suspend_gap_paused"
                handle = "0x1"
                gap_seconds = 3600
                suspend_gap_s = 60
                settle_until = $settleUntil.ToString("o")
                post_resume_settle_s = 60
                prompts_sent = 2
                session_id = "suspend-gap-test-session"
            } | ConvertTo-Json -Compress)
        ...
        try {
            $json = & $statusScript -ReceiptPath $receipt -Json
            $summary = ($json | Out-String) | ConvertFrom-Json
            if ($summary.LastSuspendGap.Active -ne $true) {
                throw "Status did not report active suspend-gap settle."
            }

            if ($summary.LastSuspendGap.GapSeconds -ne 3600) {
                throw "Status did not report suspend-gap duration."
            }
        }
        finally {
            Remove-Item -LiteralPath $receipt -Force -ErrorAction SilentlyContinue
